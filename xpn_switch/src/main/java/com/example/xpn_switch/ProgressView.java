...
public class ProgressView extends View {
    ...
    private long defaultDuration = 2 * 1000;
    private long defaultReconnectingDuration = 2 * 1000;
    ...
    private void initAnimator() {
        mAnimatorListener = new Animator.AnimatorListener() {
            ...
            public void onAnimationEnd(Animator animation) {
                ...
                } else if (mCurrentState == State.ReConnecting) {
                    if (!mReconnectingIsStop) {
                        mReConnectingValueAnimator.start();
                        mReconnectingCount++;
                        if (mReconnectingCount > 1) {
                            mReconnectingIsStop = true;
                        }
                    } else {
                        mCurrentState = State.Connected;
                        mAnmitorHandler.sendEmptyMessage(0);
        ...
        mConnectingValueAnimator = ValueAnimator.ofFloat(0, 1).setDuration(defaultDuration);
        mConnectingValueAnimator.setInterpolator(new AccelerateDecelerateInterpolator());
        mConnectingValueAnimator.addListener(mAnimatorListener);
        ...
        mReConnectingValueAnimator.addListener(mAnimatorListener);
        ...
        mAnmitorHandler = new Handler() {
            ...
            public void handleMessage(Message msg) {
                switch (mCurrentState) {
                    //断开连接
                    case Disconnect:
                        mReConnectingValueAnimator.removeAllListeners();
                        mConnectingValueAnimator.removeAllListeners();
                    ...
                    case Connecting:
                        isDrawablePath = true;
                        mConnectingValueAnimator.setInterpolator(new AccelerateInterpolator());
                        mConnectingValueAnimator.addUpdateListener(mUpdateListener);
                        mConnectingValueAnimator.addListener(mAnimatorListener);
                        //修改画笔的宽度
                        mProgressPaint.setStrokeWidth(defaultCircleWidth * 2 / 5);
